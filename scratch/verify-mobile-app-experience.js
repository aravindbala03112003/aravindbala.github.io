...
async function runVerification() {
  ...
  try {
    ...
    console.log('3. Testing Device Preset Switcher (430px, 360px)...');
    ...
    const mobileCheck = await send('Runtime.evaluate', {
      expression: `(() => {
        return {
          ...
          hasSelfIntroBtn: Boolean(document.querySelector('.nav-self-intro-btn')),
    ...
    await send('Runtime.evaluate', {
      expression: `(() => {
        const btn = document.querySelector('.nav-self-intro-btn');
        if (btn) btn.click();
      })()`
    });
    await new Promise(r => setTimeout(r, 1000));
